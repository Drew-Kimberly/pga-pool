 ...
 * `c_thumb,g_face,w_280,h_350,z_0.7`) with a face-detected fill crop at the
 * correct resolution, plus auto-format and auto-quality for modern codecs.
 *
...
export function getOptimizedHeadshotUrl(url: string, size: number): string {
  ...
  return `${CLOUDINARY_UPLOAD_PREFIX}c_fill,g_face,w_${px},h_${px},f_auto,q_auto/${assetPath}`;
}
